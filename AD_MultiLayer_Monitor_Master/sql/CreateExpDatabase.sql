...
drop table if exists service_invocation;
...
create table run (run_id int primary key auto_increment, run_type_id int,
...
create table service_invocation (service_invocation_id int primary key auto_increment, run_id int, service_id int,
	foreign key (service_id) references service(service_id) on delete cascade,
    foreign key (run_id) references run(run_id) on delete cascade);
